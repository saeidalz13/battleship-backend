...
func (sm *SessionManager) ManageCommunication() {
	for {
		...
		case TypeSessionMessageBytes:
			msgByte, ok := msg.Payload.([]byte)
			if ok {
				receiverSession.Conn.WriteMessage(websocket.TextMessage, msgByte)
			}
		
		default:
			log.Println("invalid message type for intersession communication")
			continue
		}
